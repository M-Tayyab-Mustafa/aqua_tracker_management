    ...
    on<Add>((event, emit) async {
      ...
      await showDialog(
        ...
        builder: (context) => AlertDialog.adaptive(
          ...
          actions: [
            CustomButton(
                onPressed: () => Navigator.pop(context), title: 'Cancel'),
            CustomButton(
                onPressed: () async {
                  if (formKey.currentState!.validate()) {
                    ...
                    try {
                      await firebaseCompanyDoc.then((doc) async {
                        await doc.get().then((company) async {
                          double bottlePrice = double.parse(
                              (company.data() as Map)['bottle_price']
                                  .toString());
                          for (Sale sale in sales) {
                            ...
                            if (now.day == saleDateTime.day &&
                              ...
                              sales[(sales.indexOf(sale))] = sale.copyWith(
                                  bottles: newBottles,
                                  amount: bottlePrice * newBottles);
    ...
    try {
      await firebaseCompanyDoc.then((doc) async {
        ...
        await doc
            ...
            .get()
            .then((branchDoc) {
          sales = (branchDoc.data()!['sales'] as List)
              .map<Sale>((sale) => Sale.fromMap(sale))
              .toList()
              .reversed
              .toList();
          if (sales.isNotEmpty) {
            maxYAxis = sales.map((e) => e.amount).reduce(max) +
                500; // To make sure Graph always greater then max Value
            individualBars = List.generate(
                sales.length,
                (index) => IndividualBar(
                    x: index, y: sales[index].amount, date: sales[index].date));
